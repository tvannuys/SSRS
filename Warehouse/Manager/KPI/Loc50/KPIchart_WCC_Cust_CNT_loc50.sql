...
/****** Object:  StoredProcedure [dbo].[KPIchart_WCC_Cust_CNT_loc50]    Script Date: 4/9/2014 8:40:59 AM ******/
SET ANSI_NULLS ON
GO
...
-------------------------------------------------------------
CREATE PROC [dbo].[KPIchart_WCC_Cust_CNT_loc50] AS
BEGIN
---------------------------------
--
...
-- Collect status = 'T' SHIPPED
	...
	(
	SELECT *
	FROM OPENQUERY (GSFL2K,'SELECT rfcust 
								FROM rfwillchst
								WHERE rfloc =50
	...
	SELECT COUNT(DISTINCT rfcust) as c_cnt, NULL as p_cnt
	INTO #WCC_C
...
-- Collect status = 'T' SHIPPED
	...
	SELECT COUNT(DISTINCT rfcust) as p_cnt, NULL as c_cnt
	INTO #WCC_P
